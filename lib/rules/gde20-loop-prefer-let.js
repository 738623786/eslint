...
/**
 * Checks whether a given node is located at `LoopStatement.init` or not.
 * @param {ASTNode} node A node to check.
 * @returns {boolean} `true` if the node is located at `LoopStatement.init`.
 */
function isInitOfLoopStatement(node) {
    let { parent } = node;
    let isLoop = null;

    while (isLoop === null) {
        if (["ForStatement", "ForOfStatement", "ForInStatement", "WhileStatement", "DoWhileStatement"].includes(parent.type)) {
            isLoop = true;
        }
        if (["Program", "FunctionDeclaration", "ArrowFunctionExpression"].includes(parent.type)) {
            isLoop = false;
        }
...
module.exports = {
    meta: {
        ...
        docs: {
            ...
            recommended: false,
            url: "https://eslint.org/docs/latest/rules/gde20-loop-prefer-let"
        },
        ...
        messages: {
            useLet: "'{{name}}' is defined in a loop statement. Use 'let' instead."
        }
    },
    ...
    create(context) {
                        ...
                        fix: shouldFix
                            ? fixer => {
                                ...
                                return new FixTracker(fixer, sourceCode)
                                    ...
                                    .replaceTextRange(letKeywordToken.range, "let");
                            }
                            : null
                    });
                });
        ...
        return {
            ...
            VariableDeclaration(node) {
                if (node.kind === "const" && isInitOfLoopStatement(node)) {
