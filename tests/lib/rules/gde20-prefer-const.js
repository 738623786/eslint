/**
 * @fileoverview Tests for gde20-prefer-const rule.
...
const rule = require("../../../lib/rules/gde20-prefer-const"),
...
ruleTester.run("gde20-prefer-const", rule, {
    valid: [
        "for (let a in {}) {let b = ''}",
    ...
    invalid: [
        {
            code: "bb(() => {let x = 1; foo(x)})",
            output: "bb(() => {const x = 1; foo(x)})",
            errors: [{ messageId: "useConst", data: { name: "x" }, type: "Identifier" }]
        },
        {
            code: "function cc() {let x = 1; foo(x)}",
            output: "function cc() {const x = 1; foo(x)}",
            errors: [{ messageId: "useConst", data: { name: "x" }, type: "Identifier" }]
        },
